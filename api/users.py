...
@request_json_handle(
    request,
    optional_keys=["name", "password", "user_code", "ticket", "email", "custom_banner"],
    must_change=True,
)
@api_try
...
@bp.route("/<int:user_id>/b50", methods=["GET"])
@role_required(request, ["select", "select_me"])
@api_try
def users_user_b50_get(user, user_id):
    """查询用户b50"""
    if user_id <= 0:
        return error_return(InputError(api_error_code=-110))
    # 查别人需要select权限
    ...
    with Connect() as c:
        ...
        x.select_from_user()
        if not x.scores:
            raise NoData(f"No best50 data of user `{user_id}`", api_error_code=-3)
        x.select_song_name()
        r = x.to_dict_list()
        rating_sum = sum(i.rating for i in x.scores)
        return success_return(
            {"user_id": user_id, "b50_ptt": rating_sum / 50, "data": r}
        )
