...
from .udp_class import Room, b, bi
...
class CommandParser:
    ...
    def get_commands(self, command: bytes):
        ...
        received_sequence = bi(self.command[12:16])
        player.last_command_num = max(player.last_command_num, received_sequence)
        ...
        start_sequence = max(
            received_sequence, player.start_command_num, self.room.command_queue_base
        )
        flag_13 = False
        for sequence in range(start_sequence, self.room.command_queue_length):
            command_item = self.room.get_command(sequence)
            if command_item is None:
                continue
            if command_item[2] == 0x13:
                if flag_13:
                    break
                flag_13 = True
            re.append(command_item)

        if player.extra_command_queue:
            re += player.extra_command_queue
            player.extra_command_queue = []

        if r:
            re += r

        self.room.prune_command_queue()
        return re
    ...
    def command_01(self):
        ...
        self.room.append_command(self.s.command_10())
    ...
    def command_02(self):
        ...
        if self.room.state == 2:
            ...
            self.room.append_command(self.s.command_11())
            self.room.append_command(self.s.command_13())
    ...
    def command_03(self):
        ...
        self.room.append_command(self.s.command_12(self.player_index))
        ...
        if (
            ...
            for i in range(4):
                if self.room.players[i].player_id == player_id:
                    ...
                    self.room.append_command(self.s.command_12(i))
                    self.room.append_command(self.s.command_14())
                    break
    ...
    def command_07(self):
        self.s.random_code = self.command[16:24]
        self.room.players[self.player_index].song_unlock = self.command[
            24 : 24 + Config.LINK_PLAY_UNLOCK_LENGTH
        ]
    ...
    def command_09(self):
        ...
        if bi(self.command[12:16]) == 0:
            ...
            self.room.append_command(self.s.command_15())
            return None
        ...
        if self.room.state in (7, 8):
            ...
            if player.score.timer != 0 or self.room.state != 8:
                score_command = self.s.command_0e(self.player_index)
                score_key = b(player.player_id, 8)
                for i in self.room.players:
                    i.queue_extra_command(
                        score_command, coalesce_type=0x0E, coalesce_key=score_key
                    )
    ...
    def command_0b(self):
        ...
        for i in range(4):
            if self.player_index != i and self.room.players[i].online == 1:
                self.room.players[i].queue_extra_command(
