...
class Player:
    ...
    def queue_extra_command(
        self,
        command: bytes,
        max_size: int = 12,
        coalesce_type: int = None,
        coalesce_key: bytes = None,
    ):
        if coalesce_type is not None and coalesce_key is not None:
            for index in range(len(self.extra_command_queue) - 1, -1, -1):
                old = self.extra_command_queue[index]
                if (
                    len(old) >= 24
                    and old[1] == coalesce_type
                    and old[16:24] == coalesce_key
                ):
                    self.extra_command_queue[index] = command
                    return

        self.extra_command_queue.append(command)
        if len(self.extra_command_queue) > max_size:
            del self.extra_command_queue[:-max_size]
...
class Room:
    ...
    def command_queue_length(self) -> int:
        return self.command_queue_base + len(self.command_queue)

    def append_command(self, command: bytes):
        self.command_queue.append(command)

    def get_command(self, sequence: int):
        if sequence < self.command_queue_base:
            return None
        index = sequence - self.command_queue_base
        if index >= len(self.command_queue):
            return None
        return self.command_queue[index]

    def prune_command_queue(self):
        active_players = [p for p in self.players if p.player_id != 0]
        if not active_players or not self.command_queue:
            return

        safe_sequence = min(
            max(p.start_command_num, p.last_command_num) for p in active_players
        )
        removable = safe_sequence - self.command_queue_base
        if removable <= 0:
            return

        removable = min(removable, len(self.command_queue))
        del self.command_queue[:removable]
        self.command_queue_base += removable
        ...
        with self.lock:
            ...
            for i, x in enumerate(self.players):
                ...
                if now - x.last_timestamp >= Config.PLAYER_TIMEOUT:
                    ...
                    x.online == 1
                    and now - x.last_timestamp >= Config.PLAYER_PRE_TIMEOUT
                ):
                    x.online = 0
                    player_index_list.append(i)
    ...
    def make_voting(self):
        ...
        logging.info(f"Room `{self.room_code}` votes song `{self.song_idx}`")
