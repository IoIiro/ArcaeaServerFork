...
class ItemFactory:
    ...
    def get_item(self, item_type: str):
        ...
        elif item_type == "custom_banner":
            return CustomBanner(self.c)
        else:
            raise InputError(
...
class UserItemList:
    ...
    def select_from_type(self, item_type: str) -> "UserItemList":
        ...
        if (
            ...
            and Config.WORLD_SONG_FULL_UNLOCK_WITHOUT_MAP
        ...
        if (
            ...
            or Config.ONLINE_BANNER_FULL_UNLOCK
            and item_type == "online_banner"
        ):
    ...
    def _select_world_song_without_map(self):
        ...
        """
        assert (
            Config.WORLD_SONG_FULL_UNLOCK_WITHOUT_MAP and Config.WORLD_SONG_FULL_UNLOCK
        )
        self.c.execute('''select item_id from item where type="world_song"''')
        x = self.c.fetchall()
        self.c.execute(
            """select item_id, amount from user_item where type = "world_song" and user_id = ?""",
            (self.user.user_id,),
        )
        y = self.c.fetchall()
        if not x or not y:
        ...
        for i in x:
            item_id = i[0]
            if (
                item_id in MapParser.world_song_rewards
                and item_id not in user_world_songs
            ):
                continue
